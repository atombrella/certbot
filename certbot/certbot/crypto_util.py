...
def init_save_key(key_prop, key_dir, keyname="key-certbot.pem"):
    ...
    :param int key_prop: Key property such as type, bits, curve name
    ...
    with key_f:
        key_f.write(key_pem)
    if key_prop["type"] == "rsa":
        logger.debug("Generating RSA key (%d bits): %s", key_prop["size"], key_path)
    elif key_prop["type"] == "ecdsa":
        logger.debug("Generating EC key (%s): %s", key_prop["curve"], key_path)
...
def make_key(key_prop):
    ...
    :param dict key_prop: Key property such as type, bits, curve name.
    ...
    if key_prop["type"] == "rsa":
        ...
        curve_name = key_prop["curve"]
        cv = getattr(ec, curve_name.upper())
        assert curve_name == cv.name
        ekey = ec.generate_private_key(cv(), default_backend())
        return ekey.private_bytes(
            encoding=serialization.Encoding.PEM,
            format=serialization.PrivateFormat.TraditionalOpenSSL,
            encryption_algorithm=serialization.NoEncryption()
        )
    else:
        error_str = "Unsupported key type: {0}".format(key_prop["type"])
        logger.exception(error_str)
        raise errors.Error(error_str)


def valid_privkey(privkey):
    """Is valid RSA/EC private key?
